...
- [ ] Service rows: spacing rhythm, description column type check against the
      board (serif ≥28px, one italic word, mono ≥9px UI), 2px geometry
      everywhere including the CTA form controls.
- [ ] Lower sections + Work page: radii audit (nothing >2px), no green
      status accents, no glow shadows, imagery frames square-ruled.
- [ ] CTA form: focus states, error states, keyboard flow; success message
      honest; verify at 375px.
- [ ] Footer: cross-check against the board's footer spec.
- [ ] svelte-check 0 errors; report a delta list in this ticket when done.
