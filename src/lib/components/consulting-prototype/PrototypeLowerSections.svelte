	...
	<section class="work-showcase shell" id="work" data-motion-section>
		...
		<div class="project-stage" aria-label="Selected Lutz Consulting Group work" data-motion-item>
			...
			<a class="workspace-window" href={projects[0].url} target="_blank" rel="noreferrer" aria-label={`Visit ${projects[0].name}`} onclick={() => trackOutbound(projects[0].url, 'proto_work_showcase')}>
				<div class="window-bar"><i></i><i></i><i></i><small>{projects[0].name} · {projects[0].host}</small></div>
			...
			<a class="shoe-card" href={projects[1].url} target="_blank" rel="noreferrer" aria-label={`Visit ${projects[1].name}`} onclick={() => trackOutbound(projects[1].url, 'proto_work_showcase')}>
				...
				<span>{projects[1].name} · {projects[1].host}</span>
			</a>
			<a class="mind-card" href={projects[2].url} target="_blank" rel="noreferrer" aria-label={`Visit ${projects[2].name}`} onclick={() => trackOutbound(projects[2].url, 'proto_work_showcase')}>
				<small>{projects[2].name} · {projects[2].host}</small><strong>MindLSN</strong><span>Adaptive drills · ELO · integrity checks</span>
			</a>
...
<style>
	...
	.project-stage{position:relative;height:min(62vw,780px);min-height:620px;overflow:hidden;border:1px solid var(--proto-line-strong);background:var(--proto-surface);isolation:isolate}.stage-depth{position:absolute;inset:0;background:radial-gradient(circle at 28% 26%,rgba(7,84,63,.85),transparent 47%),linear-gradient(180deg,transparent 45%,rgba(0,0,0,.4))}.workspace-window,.shoe-card,.mind-card{position:absolute;z-index:2;display:block;border:1px solid var(--proto-line-strong);box-shadow:0 30px 90px rgba(0,0,0,.42);color:inherit;text-decoration:none;transition:transform .55s cubic-bezier(.2,.75,.2,1),border-color .3s ease,box-shadow .55s ease}.workspace-window:hover,.shoe-card:hover,.mind-card:hover{z-index:5;border-color:rgba(240,239,233,.44);box-shadow:0 42px 110px rgba(0,0,0,.54)}.workspace-window:focus-visible,.shoe-card:focus-visible,.mind-card:focus-visible{z-index:5;border-color:var(--proto-brass);outline:2px solid var(--proto-brass);outline-offset:4px}.workspace-window{width:75%;height:58%;right:5%;top:16%;overflow:hidden;background:#1c1d22;transform:rotate(-1deg)}.workspace-window:hover,.workspace-window:focus-visible{transform:rotate(0) translateY(-6px)}.workspace-window img{width:100%;height:calc(100% - 34px);object-fit:cover;object-position:left top;filter:saturate(.92) brightness(.9);transition:filter .35s ease,transform .7s ease}.workspace-window:hover img,.workspace-window:focus-visible img{filter:saturate(1) brightness(1);transform:scale(1.01)}.window-bar{height:34px;display:flex;align-items:center;gap:5px;padding:0 11px;background:#1c1d22}.window-bar i{width:6px;height:1px;background:#707078}.window-bar small{margin-left:8px;color:#a1a29f;font:500 9px var(--proto-mono);text-transform:uppercase}.shoe-card{width:29%;height:44%;left:4%;bottom:6%;overflow:hidden;background:#eee8dd;transform:rotate(1.5deg)}.shoe-card:hover,.shoe-card:focus-visible{transform:rotate(.25deg) translateY(-7px)}.shoe-card img{width:100%;height:100%;object-fit:cover;object-position:center 72%;transition:transform .7s ease}.shoe-card:hover img,.shoe-card:focus-visible img{transform:scale(1.025)}.shoe-card span{position:absolute;left:0;right:0;bottom:0;padding:12px;color:#38352f;background:rgba(238,232,221,.92);font:500 9px var(--proto-mono);text-transform:uppercase}.mind-card{width:38%;right:1%;bottom:3%;padding:20px;color:#171713;background:var(--proto-paper);transform:rotate(-.7deg)}.mind-card:hover,.mind-card:focus-visible{transform:rotate(0) translateY(-7px)}.mind-card small{display:block;font:500 9px var(--proto-mono);letter-spacing:.1em;text-transform:uppercase}.mind-card strong{display:block;margin:11px 0 3px;font:400 50px var(--proto-display)}.mind-card span{color:#625e55;font-size:11px}.project-thread{position:absolute;z-index:1;left:0;right:0;top:41%;height:1px;background:rgba(180,154,103,.38)}.project-thread i{position:absolute;left:51%;top:-4px;width:9px;height:9px;background:var(--proto-brass)}
