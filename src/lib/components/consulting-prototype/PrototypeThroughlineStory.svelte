<script lang="ts">
	...
	const PAUSE = 0.16; // Jack 07-18: a beat between a click landing and the thing it does
	...
	// Tab widths in cqw (% of the screen frame). Tabs pack in from the left; only
	// the newest open tab is full-size, the rest collapse to just their logo, and
	// the "+" sits right after the last one (Jack 07-18). box-sizing:border-box
	// makes a 0-width tab truly vanish (padding included). The cursor reads the
	// live "+" and Notion tab positions each frame so it always lands on them,
	// whatever the strip reflow does.
	const TAB_FULL = 8.4;
	const TAB_ICON = 3.5;
	const NOTION_FULL = 8.4;
	const HELP_FULL = 13.5;
	// Click cadence: the cursor taps the "+" to open each tool, then the Notion
	// tab, then one fresh tab. Actions lag each tap by PAUSE.
	const toolClickTimes = [1.70, 2.22, 2.74, 3.26, 3.78, 4.30, 4.82, 5.34, 5.86, 6.38];
	const notionClick = 6.95;
	const helpClick = 13.30;
	const plusClickTimes = [...toolClickTimes, notionClick, helpClick];
	const boxTapTimes = [17.3, 17.61, 17.92, 18.23, 18.54, 18.85, 19.16, 19.47];
	let screenContentEl: HTMLElement;
	...
	const tabs = [
		...
		{ name: 'GitHub', job: 'Version control', icon: siGithub, color: '#f2f2f2' },
		{ name: 'Vercel', job: 'DNS + hosting', icon: siVercel, color: '#ffffff' },
		{ name: 'Railway', job: 'Cloud backend', icon: siRailway, color: '#a78bfa' },
		{ name: 'Supabase', job: 'Database', icon: siSupabase, color: '#3ecf8e' },
		{ name: 'Clerk', job: 'Auth', icon: siClerk, color: '#6c47ff' },
		{ name: 'Stripe', job: 'Billing', icon: siStripe, color: '#635bff' },
		{ name: 'PostHog', job: 'Analytics', icon: siPosthog, color: '#f59e0b' },
		{ name: 'Sentry', job: 'Error monitoring', icon: siSentry, color: '#a279ff' }
	];
	...
	let isFailure = $derived(story >= 12.15 && story < 13.55 || story >= 21.75 && story < 22.64);
	let isSearch = $derived(story >= 13.55 && story < 15.35);
	let isConsulting = $derived(story >= 15.35 && story < 21.75);
	let isResolved = $derived(story >= 22.64);
	let bizSite = $derived(isResolved ? heroScene.businessSite.after : heroScene.businessSite.before);
	let notionVisible = $derived(ramp(story, 7.11, 7.5) * (1 - ramp(story, 21.15, 21.55)));
	let helpVisible = $derived(ramp(story, 13.46, 13.8) * (1 - ramp(story, 21.45, 21.8)));
	let queryCharacters = $derived(Math.floor(heroScene.browser.typedUrl.length * ramp(story, 14.16, 15.16)));

	// Every click drives its own press envelope so the cursor and its target agree.
	// grabPress holds a grip through the Notion drag; taps() are seekable envelopes.
	...
	let plusPress = $derived(taps(plusClickTimes));
	let buttonPress = $derived(pressAmt(16.35));
	let refreshPress = $derived(pressAmt(22.48));
	let refreshSpin = $derived(easeOut(ramp(story, 22.64, 23.16)));
	// One ring per click. The "+" and grab rings ride the live tab positions; the
	// rest are fixed workspace targets. [t, x, y, maxScale, dur]
	let ring = $derived.by(() => {
		for (const t of plusClickTimes) {
			if (story >= t && story < t + 0.34) {
				const p = (story - t) / 0.34;
				return { x: plusTarget.x, y: plusTarget.y, s: 0.35 + 1.25 * easeOut(p), o: (1 - p) * (1 - p) * 0.8 };
			}
		}
		if (story >= 7.4 && story < 7.74) {
	...
	function tabAmount(index: number) {
		const open = toolClickTimes[index] + PAUSE; // the tab appears a beat after its "+" click
		const opened = ramp(story, open, open + 0.3);
	...
	function taskComplete(index: number) {
		const done = boxTapTimes[index] + PAUSE; // the check lands a beat after the tap
		return ramp(story, done, done + 0.2);
	}
	...
	function cursorStyle() {
		...
		const press = Math.min(
			1,
			plusPress + grabPress + pressAmt(14.0) + buttonPress + taps(boxTapTimes) + refreshPress
		);
	...
	<div class="story-player">
		<div class="monitor" role="img" aria-label="An animated browser story where a business owner hands off a complicated website stack to Lutz Consulting Group">
			...
			<div class="screen-frame" bind:this={screenFrame}>
				<div class="screen-content" bind:this={screenContentEl}>
					...
					<div class="browser">
						<div class="tab-strip">
							...
							<div class="browser-tab help-tab" class:active={isSearch || isConsulting} style={`--w:${helpWidth()};--tab-opacity:${clamp(helpVisible * 3)};--tab-y:${(1 - helpVisible) * 6}px`}>
							...
							<div class="new-tab" bind:this={plusEl} style={`--plus-press:${plusPress}`}>+</div>
						</div>
